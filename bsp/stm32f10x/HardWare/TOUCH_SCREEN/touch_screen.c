...
struct rt_spi_device *rt_spi_tp0_device;
struct rt_spi_message rt_spi_tp0_message;
struct rt_spi_device *rt_spi_tp1_device;
struct rt_spi_message rt_spi_tp1_message;
...
u8 TP_Scan(LCD_TypeDef *TFTLCD, u8 tp)
{
	...
	if(*tp_irq==0)//有按键按下
	{
		if(tp)TP_Read_XY2(TFTLCD, &tp_dev->x[0],&tp_dev->y[0]);//读取物理坐标
		else if(TP_Read_XY2(TFTLCD, &tp_dev->x[0],&tp_dev->y[0]))//读取屏幕坐标
		{
...
//保存校准参数										    
void TP_Save_Adjdata(LCD_TypeDef *TFTLCD)
{
	int fd, size;
	char buffer[14];
	_m_tp_dev *tp_dev;
	...
	{
		/* 移动文件指针到指定位置 */
		if (lseek(fd, 0 + start, SEEK_SET) == -1)
		{
			rt_kprintf("lseek %s failed\r\n", file_name);
			close(fd);
		}
		else
		{
...
{								 
	...
	while(1)//如果连续10秒钟没有按下,则自动退出
	{
		...
		if(outtime>1000)
		{
			TP_Get_Adjdata(TFTLCD);
			break;
	 	} 
 	}
}

void tp1_attach_device()
{
	...
	rt_spi_bus_attach_device(&spi_device, "SPI_TP1", "SPI3", (void*)&spi_cs);
}

void tp0_attach_device()
{
	static struct rt_spi_device spi_device;
	static struct stm32_spi_cs  spi_cs;
	GPIO_InitTypeDef  GPIO_InitStructure;
	...
	rt_spi_bus_attach_device(&spi_device, "SPI_TP0", "SPI3", (void*)&spi_cs);
}
...
u8 TP_Init(void)
{
	...
	rt_spi_tp0_device = (struct rt_spi_device *) rt_device_find("SPI_TP0");
	if (rt_spi_tp0_device == RT_NULL)
	{
		return 0;
	}
	
	rt_spi_tp1_device = (struct rt_spi_device *) rt_device_find("SPI_TP1");
	if (rt_spi_tp1_device == RT_NULL)
	{
		return 0;
	}
	...
	/* config spi */
	{
		...
		rt_spi_configure(rt_spi_tp0_device, &cfg);
	...
	for (u8 i = 0; i < 2; i++)
	{
		...
		TP_Read_XY(TFTLCD, &tp_dev->x[0],&tp_dev->y[0]);//第一次读取初始化	 
		if(TP_Get_Adjdata(TFTLCD)){}//已经校准
		else			  		//未校准?
		{ 										    
			LCD_Clear(TFTLCD, WHITE);	//清屏
