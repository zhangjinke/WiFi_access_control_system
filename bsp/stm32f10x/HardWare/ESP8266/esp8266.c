...
#if 0
#define PRINT_ESP_DEBUG_INFO
#endif

/** \brief 设置调试信息打印开关 */
#ifdef PRINT_ESP_DEBUG_INFO
#define ESP_PRINTF(fmt,args...) rt_kprintf("ESP_PRINTF: "); rt_kprintf(fmt, ##args)
#else
#define ESP_PRINTF(fmt,args...)
#endif
...
* 返回值    : None
*******************************************************************************/
void esp8266_spi_transmit(uint8_t cmd, uint8_t addr, uint8_t *buf)
{
    static uint8_t esp8266_readBuf[34],esp8266_writeBuf[34];    //SPI通信缓存
    int i;
...
* 返回值    : -1: 失败 0: 成功
*******************************************************************************/
int8_t esp8266_spi_write(uint8_t *pack, uint32_t lenth)
{
    int i,j,t = 50000;
    uint8_t buf[32];
...
int8_t WriteTest(uint32_t lenth)
{
    uint8_t buf[1024*5];
    int i;
...
uint8_t  recv_pack[1024*5]; /* 接收缓冲区 */
uint32_t recv_lenth = 0;    /* 接收到的包长度 */
uint8_t  is_recv_pack = 0;  /* 是否接收到数据包 0: 未接受 1: 已接受 */
/*******************************************************************************
...
* 返回值    : None
*******************************************************************************/
void esp8266_io_cfg(uint8_t io, uint8_t mode)
{
